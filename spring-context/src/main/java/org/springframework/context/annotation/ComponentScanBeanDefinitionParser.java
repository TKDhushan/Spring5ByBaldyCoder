...
public class ComponentScanBeanDefinitionParser implements BeanDefinitionParser {
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		//读取 <context:component-scan>标签的base-package属性
		...
		//创建并配置 ClassPathBeanDefinitionScanner
		ClassPathBeanDefinitionScanner scanner = configureScanner(parserContext, element);
		//scanner在执行的basePackageb包中扫描，返回已注册的beanDefinition
		Set<BeanDefinitionHolder> beanDefinitions = scanner.doScan(basePackages);
		//组件注册 包括注册内部的后置处理器，注册触发事件
	...
	protected void registerComponents(
		...
		//将扫描的所有beanDefinition添加到compositeDef的nestedComponent属性中
		...
		if (element.hasAttribute(ANNOTATION_CONFIG_ATTRIBUTE)) {
			//获取component-scan标签的annotation-config属性，默认为true
			annotationConfig = Boolean.parseBoolean(element.getAttribute(ANNOTATION_CONFIG_ATTRIBUTE));
		}
		if (annotationConfig) {
			//如果annotation-config属性为true.在给定的注册表中注册所有用于注解的bean后置处理器
