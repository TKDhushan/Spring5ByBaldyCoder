...
class ConditionEvaluator {
	...
	public boolean shouldSkip(@Nullable AnnotatedTypeMetadata metadata, @Nullable ConfigurationPhase phase) {
		...
		if (phase == null) {
			/**
			 * 下面逻辑校验判断中，需要进入ConfigurationClassUtils.isConfigurationCandidate方法，主要逻辑如下:
		...
		for (Condition condition : conditions) {
			...
			//requiredPhase只可能是空或者ConfigurationCondition的一个实例对象
			if ((requiredPhase == null || requiredPhase == phase) && !condition.matches(this.context, metadata)) {
				/**
				 * 此逻辑为：
				 * 1、requiredPhase不是ConfigurationCondition的实例
				 * 2、phase==requiredPhase,从上述的递归可知
				 * 		a.phase可谓ConfigurationPhase.PARSE_CONFIGURATION或者ConfigurationPhase.REGISTER_BEAN
